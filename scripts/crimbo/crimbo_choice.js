...
function getgetPeridoteOption() {
  var option = get("choiceAdventure1557");
  if (typeof option === "number") return [option, null];
  var _option$split = option.split("&", 2),
    _option$split2 = _slicedToArray(_option$split, 2),
    value = _option$split2[0],
    addendums = _option$split2[1];
  return [Number(value), addendums];
}
function runPeridotChoice(pageText) {
  var _getgetPeridoteOption = getgetPeridoteOption(),
    _getgetPeridoteOption2 = _slicedToArray(_getgetPeridoteOption, 2),
    option = _getgetPeridoteOption2[0],
    addendums = _getgetPeridoteOption2[1];
  if (!addendums) return void kolmafia.runChoice(option);
  if (!addendums.startsWith("bandersnatch=")) {
    kolmafia.abort("Invalid peridot args: ".concat(addendums));
  }
  var monsterId = addendums.slice("bandersnatch=".length);
  if (!kolmafia.xpath(pageText, "//form//input[@name='bandersnatch'][@value='".concat(monsterId, "']")).length) {
    return void kolmafia.runChoice(2); // Monster not available
  }
  return void kolmafia.runChoice(option, addendums);
}
...
exports.main = main;
exports.runPeridotChoice = runPeridotChoice;
